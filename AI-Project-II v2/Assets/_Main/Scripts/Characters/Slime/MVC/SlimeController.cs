...
namespace Game.Entities.Slime
{
    public class SlimeController : EntityController<SlimeStatesEnum>
    {
        ...
        protected override void Start()
        {
            ...
            if(!_isDataNull)
                _flocking = new SlimeFlockingManager(_data, GetModel<SlimeModel>(), Model.GetLevelable());
        ...
        {
            if(StateMachine == null) return;
            StateMachine.SetState(SlimeStatesEnum.Idle);
        }

        private void ActionMove()
        {
            if(StateMachine == null) return;
            StateMachine.SetState(SlimeStatesEnum.FollowRoute);
        }

        private void ActionPowerUp()
        {
            if (StateMachine == null) return;
            StateMachine.SetState(SlimeStatesEnum.PowerUp);
        }

        private void ActionDead()
        {
            if(StateMachine == null) return;
            StateMachine.SetState(SlimeStatesEnum.Die);
        }
        ...
        {
            return Model != null && GetModel<SlimeModel>().IsAlive();
        }
