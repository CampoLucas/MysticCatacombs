...
using Action = BehaviourTreeAsset.Runtime.Action;
...
namespace BehaviourTreeAsset.EditorUI
{
    public class NodeView : UnityEditor.Experimental.GraphView.Node, IStyleable
    {
        ...
        public NodeView(Node node) : base(AssetDatabase.GetAssetPath(Resources.Load(UxmlNodePath)))
        {
            Node = node;
            title = node.Name;
